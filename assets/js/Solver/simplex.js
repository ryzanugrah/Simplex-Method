...
var theSampleLPString = "Maximize p = 4x + 3y subject to" + cr + "2x + 3y <= 6" + cr + "-3x + 2y <= 3" + cr + "2y <= 10" + cr + "2x + 1y <= 4";



var theInstructionString = "Format: " + cr + " (1) Variable harus diawali dengan huruf." + cr + tab + tab + "    (e.g. : p, x1, var, var2...) " + cr + " (2) Untuk bilangan pecahan, variabel berada disebelah kanan konstanta." + cr + tab + tab + "    (e.g. : (1/2)x dan bukan x/2) " + cr + " (3) Setiap variabel yang digunakan harus ada di fungsi tujuan.  " + cr + tab + tab + "    (e.g. : p = 0x + 2y + 0z ) " + cr + " (4) Gunakan kata ('maximize' atau 'minimize') dan 'subject to' " + cr + " (5) Setiap fungsi pertidaksamaan harus ditulis dibaris masing-masing. " + cr + " (6) Tidak perlu menginput data batasan : x >= 0, y >= 0, z >= 0."
...
if  (TableauNumber > maxSteps) document.theSpreadsheet.expr.value = "Tidak ada solusi optimal setelah 50 step. Diberhentikan."

else if (singular) document.theSpreadsheet.expr.value = "Solusi optimal tidak ditemukan."

else

	{

	document.theSpreadsheet.expr.value = "Solusi Optimalnya Adalah: " + objectiveName + " = ";
...
var theString = "Iterasi #" + TableauNumber + cr;
...
// alert("tableau is set up");
	...
	// Option 2 // preliminary checks
		...
		if ((accuracydig == "") || (!looksLikeANumber(accuracydig))) { document.theSpreadsheet.expr.value = "Pembulatan harus dalam range 1-13."; okToRoll = false}
		...
		if (okToRoll)
			...
			if ((thenum < 1) || (thenum > 14)) {document.theSpreadsheet.expr.value = "Pembulatan harus dalam range 1-13."; okToRoll = false}
		...
		if (document.theSpreadsheet.input.value == "") {document.theSpreadsheet.expr.value = "Masukkan SPL terlebih dahulu (atau tekan tombol Example)."; okToRoll = false; }
	...
	// 
		...
		document.theSpreadsheet.expr.value = "Tekan 'Solve' untuk mencari hasil solusi.";
